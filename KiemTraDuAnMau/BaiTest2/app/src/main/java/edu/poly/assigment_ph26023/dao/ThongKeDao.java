...
public class ThongKeDao {
    ...
    public ArrayList<Top> getTopUserASC() {
        ArrayList<Top> list = new ArrayList<>();
        String sql = "SELECT maTT, COUNT(*) AS soLuong FROM PhieuMuon " +
                "GROUP BY maTT ORDER BY soLuong ASC LIMIT 5";
        Cursor cursor = db.rawQuery(sql, null);
    ...
    public ArrayList<Top> getTopUserDESC() {
        ArrayList<Top> list = new ArrayList<>();
        String sql = "SELECT maTT, COUNT(*) AS soLuong FROM PhieuMuon " +
                "GROUP BY maTT ORDER BY soLuong DESC LIMIT 5";
        Cursor cursor = db.rawQuery(sql, null);
    ...
    public boolean getInDamPhieuMuon(String data) {
        Boolean check = false;
        Cursor cursor = db.rawQuery("SELECT maSach, count(maSach) FROM PhieuMuon " +
                "WHERE maSach = ? GROUP BY maSach", new String[]{data});
